...
<div class="itemsSorted">
	...
	for ($i = 0; $i < count($items); $i++) { 
		if($items[$i]['category'] == $_GET['from_category'] && $items[$i]['sub_category'] == $_GET['to']){
			...
			echo '<div class="button"><span class="price">'. number_format($items[$i]['price'], 2, ',', ' ') . ' '. $items[$i]['currency'] . '</span> <a href="">Upravit</a></div>';
...
<?php
	echo '<div class="add"><a href="?add_item='. $_GET['from_category'] .'&subcategory='. $_GET['to'] .'" class="add">Přidat produkt</a></div>';
?>